      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
            ...
            {order.items.map((item, index) => (
              <div key={index} className="flex justify-between">
                <div>
                  ...
                  {item.special_instructions && <p className="text-sm text-gray-500">{item.special_instructions}</p>}
                </div>
        ...
        <div className="card p-6">
          ...
          {order.status === 'delivered' && !order.has_review && (
            <div className="card p-6">
              <button onClick={() => navigate(`/orders/${id}/review`)} className="w-full btn-primary py-3">
                Leave a Review
              </button>
            </div>
          )}
